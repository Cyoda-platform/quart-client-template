...
class ComplianceValidationCriterion(CyodaCriterion):
    ...
    async def evaluate(self, entity: CyodaEntity, **kwargs: Any) -> bool:
        ...
        try:
            ...
            if not self._check_entity_reference(compliance):
                self.logger.warning(
                    f"Compliance {compliance.compliance_id} has invalid entity reference"
                )
                return False
            ...
            if not self._check_check_type(compliance):
                self.logger.warning(
                    f"Compliance {compliance.compliance_id} has invalid check type"
                )
                return False
            ...
            if not self._check_regulation(compliance):
                self.logger.warning(
                    f"Compliance {compliance.compliance_id} has invalid regulation"
                )
                return False
    ...
    def _check_entity_reference(self, compliance: Compliance) -> bool:
        ...
        return bool(
            compliance.entity_type in ["ORDER", "POSITION", "PORTFOLIO"]
            and compliance.entity_id
            and compliance.account_id
        )
    ...
    def _check_regulation(self, compliance: Compliance) -> bool:
        ...
        return bool(compliance.regulation and compliance.requirement)
