...
class MarketDataValidationCriterion(CyodaCriterion):
    ...
    async def evaluate(self, entity: CyodaEntity, **kwargs: Any) -> bool:
        ...
        try:
            ...
            if not self._check_price_consistency(market_data):
                self.logger.warning(
                    f"Market data for {market_data.symbol} failed price check"
                )
                return False
            ...
            if not self._check_volume_consistency(market_data):
                self.logger.warning(
                    f"Market data for {market_data.symbol} failed volume check"
                )
                return False
    ...
    def _check_price_consistency(self, market_data: MarketData) -> bool:
        ...
        return (
            ...
            and market_data.low_price <= market_data.high_price
            and market_data.bid_price > 0
            and market_data.ask_price > 0
        )
