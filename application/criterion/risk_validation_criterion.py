...
class RiskValidationCriterion(CyodaCriterion):
    ...
    async def evaluate(self, entity: CyodaEntity, **kwargs: Any) -> bool:
        ...
        try:
            ...
            if not self._check_limit_config(risk_control):
                self.logger.warning(f"Risk control {risk_control.risk_id} has invalid limit config")
                return False
            
            # Check threshold consistency
            if not self._check_threshold_consistency(risk_control):
                self.logger.warning(f"Risk control {risk_control.risk_id} has inconsistent thresholds")
                return False
            
            # Check action configuration
            if not self._check_action_config(risk_control):
                self.logger.warning(f"Risk control {risk_control.risk_id} has invalid action config")
                return False
    ...
    def _check_limit_config(self, risk_control: RiskControl) -> bool:
        ...
        return (
            risk_control.limit_value > 0 and
            risk_control.risk_type in [
    ...
    def _check_threshold_consistency(self, risk_control: RiskControl) -> bool:
        ...
        return (
            0 <= risk_control.warning_threshold <= 100 and
            0 <= risk_control.critical_threshold <= 100 and
            risk_control.warning_threshold <= risk_control.critical_threshold
        )
