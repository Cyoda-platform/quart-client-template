...
class Order(CyodaEntity):
    ...
    price: Optional[float] = Field(None, ge=0, description="Limit price for limit orders")
    stop_price: Optional[float] = Field(None, ge=0, description="Stop price for stop orders")
    
    # Order status and execution
    status: str = Field(default="pending", description="Order status")
    filled_quantity: float = Field(default=0, ge=0, description="Quantity filled")
    average_fill_price: Optional[float] = Field(None, ge=0, description="Average execution price")
    
    # Risk and compliance
    account_id: str = Field(..., description="Trading account ID")
    trader_id: str = Field(..., description="Trader identifier")
    risk_limit_check: bool = Field(default=False, description="Risk limit validation passed")
    compliance_check: bool = Field(default=False, description="Compliance validation passed")
    ...
    submitted_at: Optional[str] = Field(None, alias="submittedAt", description="Submission timestamp")
    executed_at: Optional[str] = Field(None, alias="executedAt", description="Execution timestamp")
    
    # Metadata
    execution_venue: Optional[str] = Field(None, description="Execution venue/exchange")
    execution_algorithm: Optional[str] = Field(None, description="Execution algorithm used")
