...
class Order(CyodaEntity):
    ...
    # Status tracking
    status: str = Field(default="PENDING", description="Order status (PENDING, ACCEPTED, REJECTED, FILLED, CANCELLED)")
    filled_quantity: float = Field(default=0.0, description="Filled quantity")
    
    # Internal workflow fields
    validation_status: Optional[str] = Field(default=None, description="Internal validation status: PASS/FAIL")
