...
class ExecutionSettlementProcessor(CyodaProcessor):
    ...
    async def process(self, entity: CyodaEntity, **kwargs: Any) -> CyodaEntity:
        ...
        """
        try:
            self.logger.info(f"Settling execution {getattr(entity, 'technical_id', '<unknown>')}")

            report = cast_entity(entity, ExecutionReport)

            # Calculate settlement amount
            settlement_amount = report.fill_quantity * report.fill_price + report.commission

            self.logger.info(f"Execution {report.technical_id} settled: ${settlement_amount}")
            return report
