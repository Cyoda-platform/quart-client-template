    ...
    async def process(self, entity: CyodaEntity, **kwargs: Any) -> CyodaEntity:
        try:
            ...
            search_req = (
                ...
                .equals("account_id", order.account_id)
                .build()
            )
            accounts: List[EntityResponse] = await entity_service.search(
                "Account", search_req
            )
            ...
            if not accounts:
                ...
                if not account.is_active:
                    ...
                    limits: List[EntityResponse] = await entity_service.search(
                        "RiskLimit", search_limit
                    )
            ...
            self.logger.info(
                f"Order {order.order_id} validation result: {validation_status}"
            )
            return order
