...
class TradeProcessor(CyodaProcessor):
    ...
    async def process(self, entity: CyodaEntity, **kwargs: Any) -> CyodaEntity:
        try:
            ...
            # 1. Update Order
            # Find order by business ID or assuming order_id is business ID?
            # Trade.order_id links to Order.order_id (business ID).
            search_order = SearchConditionRequest.builder().equals("order_id", trade.order_id).build()
            orders_resp: List[EntityResponse] = await entity_service.search("Order", search_order)
            ...
            if orders_resp:
                ...
                # Save Order update
                # We need to construct dict for save/update
                # Using update() with technical ID is best
                ...
                self.logger.info(f"Updated Order {trade.order_id} status to {new_status}")

            else:
                self.logger.warning(f"Order {trade.order_id} not found for Trade {trade.trade_id}")

            # 2. Update Position
            # Find position for account + instrument
            # We need account_id from somewhere. Trade doesn't have it?
            # Requirements: "Trade: {trade_id, order_id, ...}". "Order: {account_id...}"
            # So we get account_id from Order.
            ...
            if orders_resp:
                ...
                if positions_resp:
                    ...
                    self.logger.info(f"Created new Position for {account_id} {trade.instrument_id}")
            ...
            self.logger.error(f"Error processing trade {getattr(entity, 'technical_id', 'unknown')}: {str(e)}")
            raise
