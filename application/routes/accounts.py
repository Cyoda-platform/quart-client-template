"""
Account Routes for Trading Platform
...
async def create_account(data: Account) -> ResponseReturnValue:
    ...
    try:
        ...
        logger.info(f"Created account with ID: {response.metadata.id}")
...
async def list_accounts() -> ResponseReturnValue:
    ...
    try:
        ...
        accounts = [_to_entity_dict(item) for item in response.data]
        return {"accounts": accounts, "total": len(accounts)}, 200
    except Exception as e:
