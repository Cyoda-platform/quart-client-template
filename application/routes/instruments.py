"""
Instrument Routes for Cyoda Trading Platform

Manages all Instrument-related API endpoints including CRUD operations
for tradable financial instruments (equities, futures, options).
"""
...
from ..entity.instrument import Instrument  # noqa: F401
...
instruments_bp = Blueprint(
    "instruments", __name__, url_prefix="/api/instruments"
)
...
async def create_instrument() -> ResponseReturnValue:
    """Create a new Instrument with comprehensive validation"""
    try:
        ...
        logger.warning("Validation error creating Instrument: %s", str(e))
        return {"error": str(e), "code": "VALIDATION_ERROR"}, 400
    except Exception as e:  # pragma: no cover
        logger.exception("Error creating Instrument: %s", str(e))
        return {"error": str(e), "code": "INTERNAL_ERROR"}, 500


@instruments_bp.route("/<entity_id>", methods=["GET"])
async def get_instrument(entity_id: str) -> ResponseReturnValue:
    """Get Instrument by ID with validation"""
    try:
        ...
        logger.exception("Error getting Instrument %s: %s", entity_id, str(e))
        return {"error": str(e), "code": "INTERNAL_ERROR"}, 500


@instruments_bp.route("", methods=["GET"])
async def list_instruments() -> ResponseReturnValue:
    """List Instruments with optional filtering and validation"""
    try:
...
async def update_instrument(entity_id: str) -> ResponseReturnValue:
    """Update Instrument and optionally trigger workflow transition with validation"""
    try:
        # Validate entity ID format
        if not entity_id or len(entity_id.strip()) == 0:
            return (
        ...
        logger.info("Updated Instrument %s", entity_id)
        ...
        logger.warning(
            "Validation error updating Instrument %s: %s", entity_id, str(e)
        )
        return jsonify({"error": str(e), "code": "VALIDATION_ERROR"}), 400
    except Exception as e:  # pragma: no cover
        logger.exception("Error updating Instrument %s: %s", entity_id, str(e))
        return jsonify({"error": str(e), "code": "INTERNAL_ERROR"}), 500


@instruments_bp.route("/<entity_id>", methods=["DELETE"])
async def delete_instrument(entity_id: str) -> ResponseReturnValue:
    """Delete Instrument with validation"""
    try:
        # Validate entity ID format
        if not entity_id or len(entity_id.strip()) == 0:
            return (
        ...
        logger.info("Deleted Instrument %s", entity_id)
        ...
        logger.exception("Error deleting Instrument %s: %s", entity_id, str(e))
        return {"error": str(e), "code": "INTERNAL_ERROR"}, 500
