"""
MarketData Routes for Cyoda Trading Platform
...
from ..entity.market_data import MarketData  # noqa: F401
...
market_data_bp = Blueprint(
    "market_data", __name__, url_prefix="/api/market-data"
)
...
async def create_market_data() -> ResponseReturnValue:
    """Create a new MarketData entry with comprehensive validation"""
    try:
...
async def get_market_data(entity_id: str) -> ResponseReturnValue:
    """Get MarketData by ID with validation"""
    try:
...
async def list_market_data() -> ResponseReturnValue:
    """List MarketData entries with optional filtering and validation"""
    try:
...
async def update_market_data(entity_id: str) -> ResponseReturnValue:
    """Update MarketData and optionally trigger workflow transition with validation"""
    try:
        # Validate entity ID format
        if not entity_id or len(entity_id.strip()) == 0:
            return (
        ...
        logger.warning(
            "Validation error updating MarketData %s: %s", entity_id, str(e)
        )
...
async def delete_market_data(entity_id: str) -> ResponseReturnValue:
    """Delete MarketData with validation"""
    try:
        # Validate entity ID format
        if not entity_id or len(entity_id.strip()) == 0:
            return (
