...
Manages all MarketData-related API endpoints.
"""
...
@market_data_bp.route("/<entity_id>/transitions", methods=["GET"])
@tag(["market-data"])
@operation_id("get_market_data_transitions")
@validate(responses={200: (Dict[str, Any], None), 500: (Dict[str, Any], None)})
async def get_market_data_transitions(entity_id: str) -> ResponseReturnValue:
    """Get available transitions for MarketData"""
    try:
