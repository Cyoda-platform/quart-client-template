...
model_versions_bp = Blueprint("model_versions", __name__, url_prefix="/api/model-versions")
...
@validate(responses={200: (dict, None), 404: (dict, None), 500: (dict, None)})
async def update_model_version(entity_id: str, data: ModelVersion) -> ResponseReturnValue:
    try:
        entity_data = data.model_dump(by_alias=True)
