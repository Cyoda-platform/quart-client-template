...
async def get_order(entity_id: str) -> Tuple[Dict[str, Any], int]:
    ...
    """
    entity_service = get_entity_service()
    order = await entity_service.get_by_id(entity_id, Order.ENTITY_NAME, Order.ENTITY_VERSION)
    if order:
...
async def update_order(entity_id: str) -> Tuple[Dict[str, Any], int]:
    ...
    entity_service = get_entity_service()
    order = await entity_service.execute_transition(entity_id, transition, Order.ENTITY_NAME, Order.ENTITY_VERSION)
    if order:
        return order.data.model_dump(), 200
    return {}, 404
