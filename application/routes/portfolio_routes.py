...
async def create_portfolio() -> Tuple[Dict[str, Any], int]:
    ...
    portfolio = await entity_service.save(
        data, Portfolio.ENTITY_NAME, Portfolio.ENTITY_VERSION
    )
...
async def get_portfolio(entity_id: str) -> Tuple[Dict[str, Any], int]:
    ...
    """
    entity_service = get_entity_service()
    portfolio = await entity_service.get_by_id(
        entity_id, Portfolio.ENTITY_NAME, Portfolio.ENTITY_VERSION
    )
    if portfolio:
