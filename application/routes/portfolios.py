...
async def get_portfolio_transitions(entity_id: str) -> ResponseReturnValue:
    ...
    try:
        ...
        logger.exception(
            "Error getting transitions for Portfolio %s: %s", entity_id, str(e)
        )
        return jsonify({"error": str(e)}), 500
...
async def trigger_portfolio_transition(entity_id: str) -> ResponseReturnValue:
    ...
    try:
        ...
        logger.info(
            "Executed transition '%s' on Portfolio %s", transition_name, entity_id
        )
        return (
            jsonify(
                {
        ...
        logger.exception(
            "Error executing transition on Portfolio %s: %s", entity_id, str(e)
        )
        return jsonify({"error": str(e)}), 500
