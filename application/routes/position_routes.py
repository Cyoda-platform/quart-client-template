...
async def create_position() -> Tuple[Dict[str, Any], int]:
    ...
    position = await entity_service.save(data, Position.ENTITY_NAME, Position.ENTITY_VERSION)
...
async def get_position(entity_id: str) -> Tuple[Dict[str, Any], int]:
    ...
    """
    entity_service = get_entity_service()
    position = await entity_service.get_by_id(entity_id, Position.ENTITY_NAME, Position.ENTITY_VERSION)
    if position:
...
async def update_position(entity_id: str) -> Tuple[Dict[str, Any], int]:
    ...
    entity_service = get_entity_service()
    position = await entity_service.execute_transition(entity_id, transition, Position.ENTITY_NAME, Position.ENTITY_VERSION)
    if position:
        return position.data.model_dump(), 200
    return {}, 404
