...
async def create_position(data: Position) -> ResponseReturnValue:
    ...
    try:
        ...
        logger.info(f"Created position with ID: {response.metadata.id}")
...
async def list_positions() -> ResponseReturnValue:
    ...
    try:
        ...
        positions = [_to_entity_dict(item) for item in response.data]
        return {"positions": positions, "total": len(positions)}, 200
    except Exception as e:
