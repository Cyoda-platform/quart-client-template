...
risk_event_routes = Blueprint(
    "risk_event_routes", __name__, url_prefix="/api/risk_events"
)
...
async def create_risk_event() -> Tuple[Dict[str, Any], int]:
    ...
    risk_event = await entity_service.save(
        data, RiskEvent.ENTITY_NAME, RiskEvent.ENTITY_VERSION
    )
...
async def get_risk_event(entity_id: str) -> Tuple[Dict[str, Any], int]:
    ...
    """
    entity_service = get_entity_service()
    risk_event = await entity_service.get_by_id(
        entity_id, RiskEvent.ENTITY_NAME, RiskEvent.ENTITY_VERSION
    )
    if risk_event:
