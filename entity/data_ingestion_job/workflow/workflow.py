...
def ingest_data_process(meta, data):
    ...
    try:
        response_data = ingest_data(
            ...
            country=data.get("parameters", {}).get("country"),
            name=data.get("parameters", {}).get("name"),
        )
        raw_data_entity_id = entity_service.add_item(
            meta["token"], "raw_data_entity", ENTITY_VERSION, response_data
        )
        data["raw_entity_id"] = raw_data_entity_id
...
def aggregate_data_process(meta, data):
    ...
    try:
        ...
        data["aggregated_entity_id"] = aggregated_data_entity_id
...
class TestDataIngestionJob(unittest.TestCase):
    ...
    def test_ingest_data_process(self, mock_entity_service, mock_ingest_data):
        mock_ingest_data.return_value = [{"brpCode": "7080005051286", "country": "FI"}]
        mock_entity_service.return_value = "raw_data_entity_id"
        meta = {"token": "test_token"}
        data = {"parameters": {"code": "7080005051286", "country": "FI"}}
        ingest_data_process(meta, data)
        mock_entity_service.assert_called_once()

    @patch("common.app_init.ai_service.ai_chat")
    @patch("common.app_init.entity_service.add_item")
    def test_aggregate_data_process(self, mock_entity_service, mock_ai_chat):
        mock_ai_chat.return_value = '{"aggregated_data": "dummy_data"}'
