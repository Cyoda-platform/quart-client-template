...
def ingest_data_process(meta, data):
    ...
    try:
        response_data = ingest_data(
            code=data["request_parameters"].get("code"),
            country=data["request_parameters"].get("country"),
            name=data["request_parameters"].get("name"),
        )
...
def aggregate_data_process(meta, data):
    ...
    try:
        ...
        aggregated_data = IAiAssistantService.ai_chat(
            token=meta["token"],
...
def send_report_process(meta, data):
    logger.info("Starting process to send report.")
    try:
        ...
        logger.error(f"Error in send_report_process: {e}")
        raise
...
class TestDataProcessingJob(unittest.TestCase):
    ...
    @patch("entity.raw_data_entity.connections.ingest_data")
    ...
    def test_send_report_process(self, mock_get_item, mock_entity_service):
