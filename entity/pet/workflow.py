...
async def process_pet(entity: Dict[str, Any]) -> Dict[str, Any]:
    ...
    await process_add_processed_at(entity)
    await process_normalize_status(entity)
    await process_set_mock_age(entity)
    await process_add_default_description(entity)
    await process_normalize_id(entity)
    return entity

async def process_add_processed_at(entity: Dict[str, Any]):
...
async def process_set_mock_age(entity: Dict[str, Any]):
    age = entity.get("age")
    if not isinstance(age, int) or age < 0:
        entity["age"] = 3  # default mock age

async def process_add_default_description(entity: Dict[str, Any]):
