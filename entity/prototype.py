...
# In-memory local caches (async-safe usage by design since no concurrency control)
# Keys: search_id (str) -> pet search results (list of dicts)
_search_results_cache = {}

# Predefined fun pet facts (can be expanded or fetched from external APIs)
...
async def pets_search():
    """Trigger pet search with filters, fetch from Petstore API, store results, return search_id."""
    ...
    # Initialize cache entry with processing status
...
async def _process_pet_search(search_id: str, pet_type: str, status: str, tags: list):
    try:
        # Step 1: Fetch pets from Petstore API by status (Petstore supports status filtering)
        # Petstore API docs: GET /pet/findByStatus?status=available,sold,pending
        # Note: Petstore API does not support filtering by type or tags directly; we filter client-side.
        ...
        # Step 2: Filter results by type and tags if specified
        filtered_pets = []
        for pet in pets:
        ...
        # Step 3: Cache the filtered results with status 'completed'
...
async def pets_results(search_id):
    """Retrieve cached pet search results by search_id."""
...
async def pets_random_fact():
    """Return a fun random pet fact or recommendation."""
    try:
        # TODO: Could extend to fetch live data/facts from an external service or combine with Petstore data
        import random
...
async def before_serving():
    # Ensure the AsyncClient is ready (optional)
