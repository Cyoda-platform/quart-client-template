...
async def fetch_catfact():
    ...
    # Get JSON body but ignore content, as no params required (per spec)
    try:
        _ = await request.get_json(force=True, silent=True)
    except Exception:
        # ignore malformed or absent body, since none required
        pass
...
async def get_job_status(jobId: str):
    """
    Optional: GET endpoint to check status of a fetch job.
    Not specified in original spec but useful for UX to track async jobs.
    """
