...
# In-memory storage for bugs and comments
# Structure:
# bugs: bug_id -> bug dict
# comments: bug_id -> list of comment dicts
...
def validate_bug_input(data: dict, update: bool = False):
    # Minimal validation for required fields on create
    if not update:
        ...
        # steps_to_reproduce is optional but if present must be string
        ...
        # On update, fields are optional but if present must be valid
...
async def update_bug(bug_id):
    data = await request.get_json(force=True)
    validate_bug_input(data, update=True)
    async with bugs_lock:
        bug = bugs.get(bug_id)
        if not bug:
            abort(404, "Bug not found")
        ...
        if "status" in data and data["status"] != bug["status"]:
            ...
            updated = True
        if "description" in data and data["description"].strip() != bug["description"]:
            bug["description"] = data["description"].strip()
            updated = True
        if "severity" in data and data["severity"] != bug["severity"]:
            bug["severity"] = data["severity"]
            updated = True
        if "steps_to_reproduce" in data and data["steps_to_reproduce"].strip() != bug["steps_to_reproduce"]:
            bug["steps_to_reproduce"] = data["steps_to_reproduce"].strip()
            updated = True
        if updated:
            ...
            logger.info(f"Bug {bug_id} update called but no changes applied")
...
async def list_bugs():
    ...
    def bug_matches(bug):
        ...
        if severity_filter and bug["severity"] != severity_filter:
            return False
        if search:
            if search not in bug["title"].lower() and search not in bug["description"].lower():
                return False
        return True
    ...
    if sort_by in {"created_at", "severity", "status"}:
        # For severity sorting: define order low < medium < high
        if sort_by == "severity":
            severity_order = {"low": 0, "medium": 1, "high": 2}

            def key_func(b):
                return severity_order.get(b["severity"], -1)

        else:
            def key_func(b):
                return b.get(sort_by, "")
    ...
    # Prepare response minimal fields
    bugs_list = [
        {
            "bug_id": b["bug_id"],
            "title": b["title"],
...
# TODO: No external APIs specified in requirements, so none are used here.
# If external API integration is needed, implement using httpx.AsyncClient here.
