...
async def pets_filter(data: PetFilterRequest):
    ...
    crit = data.filter
    if crit.category or crit.status:
        def match(pet: Dict) -> bool:
            if crit.category and pet.get("category", {}).get("name", "").lower() != crit.category.lower():
                return False
            if crit.status and pet.get("status", "").lower() != crit.status.lower():
                return False
            return True
