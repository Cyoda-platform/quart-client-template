...
# Workaround: quart-schema defect requires validation last for POST and first for GET
...
@dataclass
class RandomFactBody:
    pass

@dataclass
class FavoriteAddBody:
    id: int
...
async def pets_search(data: PetSearch):
    logger.info(f"Searching pets with type={data.type} and name={data.name}")
    pets = await fetch_petstore_pets()
    filtered = filter_pets(pets, data.type, data.name)
    return jsonify({"pets": filtered})

@app.route("/pets/random-fact", methods=["POST"])
@validate_request(RandomFactBody)
async def random_fact(data: RandomFactBody):
    import random
...
@validate_request(FavoriteAddBody)
async def add_favorite(data: FavoriteAddBody):
    pet_id = data.id
