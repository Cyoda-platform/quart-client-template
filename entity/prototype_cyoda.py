...
async def search_restaurants(data: SearchRequest):
    ...
    try:
        # Add entity to external service
        # We cannot pass UUID as technical_id here, will get it from add_item response, but we want to use search_id as technical_id
        # So create item with id = search_id by adding it as part of entity data, then update local cache with this search_id
        # Since add_item returns a technical_id, but we want to keep search_id consistent, we will store search_id in entity and use it as technical_id in update calls
        # So instead of relying on returned id from add_item, we will treat search_id as technical_id for update
        # But the spec says we cannot retrieve immediately, so we just store search_id in cache and proceed
