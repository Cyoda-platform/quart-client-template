...
async def retrieve_pet_details(data: PetIdRequest):
    ...
    try:
        # Try to add a new item with technical_id as pet_id, but since add_item returns id only,
        # and we cannot specify technical_id here, we rely on separate update after fetch.
        # So we add an item with petId and status processing.
        # Note: add_item returns id, which we won't use to get data immediately.
