...
async def process_hello_message(entity_job: dict, data: dict, job_id: str):
    ...
    Stores the final greeting as an entity in entity_service.
    """
    try:
        name = data.get("name")
        if name:
        ...
        # Store the greeting message as an entity with entity_service
        entity_name = "hello_message"
        item = {"message": message, "createdAt": datetime.utcnow().isoformat()}
        try:
            # Add item to external entity_service, get technical id
            ...
            # Save id and mark job completed
            ...
            logger.info(f"Greeting processed and saved with id {item_id}: {message}")
        except Exception as e:
