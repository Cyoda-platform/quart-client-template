...
async def get_search_results(search_id):
    try:
        ...
        # result is the stored pet search request entity, but we want the search results
        # In original code, results were cached in AsyncCache.
        # Now we assume results are stored inside the entity or must be retrieved separately.
        # Since entity_service only stores the entity, let's assume the entity contains the search filters,
        # and we do a fresh query on external API here to get current pets.
        # Alternatively, if the entity contains results, return them directly.
        # We'll check if 'pets' key exists in result, else query fresh.
        if "pets" in result and result["pets"] is not None:
            pets = result["pets"]
        else:
            # fallback: query pets again with filters from stored entity
...
async def adopt_pet(data: AdoptPetRequest):
    ...
    try:
        # get all petsearchrequest items
        ...
        for search in all_searches:
            pets = None
            # If 'pets' stored inside entity, use it
            if "pets" in search and search["pets"]:
                pets = search["pets"]
            else:
                # else query pets using stored filters
