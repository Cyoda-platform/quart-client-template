...
async def process_weather_fetch_job(entity: dict) -> dict:
    ...
    await process_process_locations(entity)
    await process_finalize_job(entity)
    return entity
...
async def process_process_locations(entity: dict):
    locations = entity.get("locations", [])
    ...
    for loc in locations:
        ...
        if lat is None or lon is None:
            ...
            continue
        key = f"{lat}_{lon}"
        # Simulate cache update/add by setting value directly in entity cache dict
        entity["cache"][key] = {
            "latitude": lat,
            "longitude": lon,
...
async def process_finalize_job(entity: dict):
    entity["status"] = "done"
